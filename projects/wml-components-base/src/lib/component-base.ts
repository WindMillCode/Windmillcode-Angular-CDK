...
import { ReplaySubject, Subject, filter, interval, map, of, switchMap, takeUntil, takeWhile, tap, timer } from "rxjs"
...
export function WMLComponentBaseZeroPropsMixin<TBase extends Constructor>(
  ...
  return class   extends Base   {
    ...
    constructor(props:Partial<TBase>= new Base()) {
      super( );
      // @ts-ignore
      let origParams = Object.entries(props)
      .filter(([key, val]) => !key.startsWith('param') );
...
export class WMLComponentBaseZero<T extends WMLComponentBaseZeroProps = WMLComponentBaseZeroProps > {
  // TODO A
  @Input("params") @NotifyOnChange() props?: T | any;
