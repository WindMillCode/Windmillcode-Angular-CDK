...
export class TemplateScrollParamsSchema {
    name!:string
...
let importsToAdd  = [
  {
    ...
    importStatement: "import { ScrollPageParams } from '@core/utility/scroll-utils';\n",
    importMembers: ["ScrollPageParams"]
  },
  {
...
export function generateFn(options:TemplateScrollParamsSchema):Rule{
  return (tree:Tree)=>{
...
let updateClassFile = (options:TemplateScrollParamsSchema)=>{
  return (tree:Tree)=>{
...
function updateCpntFile(options: TemplateScrollParamsSchema,tree:Tree,targetFilePath:string) {
...
function updateTargetProperties(options: TemplateScrollParamsSchema, targetFilePath: string, tree:Tree, lastProperty: ts.PropertyDeclaration, cpntClassVar: ts.ClassDeclaration) {
...
function updateTargetMethods(
  ...
  targetStringName: CreateStringObjectType,options:TemplateScrollParamsSchema) {
  let sourceFile = wmlCreateSourceFile(targetFilePath, tree);
  let cpntClassVar = getCpntClassVar(sourceFile);
  let nameString = createStringObject(options.name)
  let fnsSnippet = `
...
init${nameString.classify(false)}GetScrollEntities = ()=>{
  ...
  this.${serviceStringName.camelCase(false)}.${serviceMethodScrollPage} = new ScrollPageParams({
...
// aux
function getServiceInfo(tree:Tree, options: TemplateScrollParamsSchema) {
