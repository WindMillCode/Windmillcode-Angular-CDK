...
export function wmlFindChildNode(
  ...
  [findOrFilter](((x:ParameterSyntaxType) => {
    if(printChildInfo){
      getTsMemberInfo(x)
    }
...
export class FindNodeTypeByRecursionParams {
  constructor(params: Partial<FindNodeTypeByRecursionParams> = {}) {
  ...
  node!: ts.Node
  kind!: ts.SyntaxKind | ((node: ParameterSyntaxType,level:number) => boolean)
  printChildInfo=false
}

export function findNodeTypeByRecursion(params: FindNodeTypeByRecursionParams,level=0): ts.Node[] {
  const foundNodes: ts.Node[] = [];

  if (params.printChildInfo) {
    getTsMemberInfo(params.node);
    console.log("At level :", level)
  }

  if (params.kind instanceof Function) {
    if (params.kind(params.node,level)) {
      foundNodes.push(params.node);
    }
  } else if (params.node.kind === params.kind) {
    foundNodes.push(params.node);
  }

  // Recursively search in child nodes
  ts.forEachChild(params.node, (childNode) => {
    const childParams = { ...params, node: childNode };
    const childFoundNodes = findNodeTypeByRecursion(childParams,level+1);
