...
import { CreateStringObjectType, DevEnvFile, FindNodeTypeByRecursionParams, WMLschematicMsgs, addImportStatements, createStringObject,  findNodeTypeByRecursion,  getIndentation, getServiceClassVar, getTsMemberInfo, performDependencyInjection, prepareForUpdate, wmlCreateSourceFile, wmlFindChildNode } from "../utils/utils";
...
function addNeededImportsToServiceClass(
  ...
  if(options.transformationType ==="scrolling"){
    importsToAdd.push({
      ...
      importStatement:`import { listEntitiesFail, ScrollPageParams } from '@core/utility/scroll-utils';`,
      importMembers:[
        'listEntitiesFail','ScrollPageParams'
      ]
    })
    importsToAdd.push({
      packageName:'rxjs',
...
${scrollingNameString.orig}ScrollPageObj:ScrollPageParams<any>

_${serviceMethodString.orig} = (uiBody = new ${serviceMethodString.capitalize(false)}UIRequestBody(),scrollPage?:ScrollPageParams)=>{
...
function addRequireBlockToSpecFile(
  ...
  let requireBlocks = findNodeTypeByRecursion(new FindNodeTypeByRecursionParams({
    node: sourceFile,
    kind: (node) => {
...
function addMockAPIResultToSpecFile(
  ...
  let targets = findNodeTypeByRecursion(new FindNodeTypeByRecursionParams({
    node: sourceFile,
    kind: (node) => {
...
spyOn(${serviceMethodString.orig}, '${serviceMethodString.orig}Load')
  ...
  if(options.transformationType === "scrolling"){
    ...
    let spyIsPresent = findNodeTypeByRecursion(new FindNodeTypeByRecursionParams({
      node: sourceFile,
      kind: (node) => {
