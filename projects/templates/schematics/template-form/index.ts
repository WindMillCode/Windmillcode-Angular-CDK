...
let importsToAdd = [
  ...
  {
    packageName: "@windmillcode/angular-wml-button-zero",
    importStatement: "import { WmlButtonOneParams, WMLButtonParamsTypeEnum } from '@windmillcode/angular-wml-button-zero';\n",
    importMembers: ["WmlButtonOneParams", "WMLButtonParamsTypeEnum"]
  },
  {
...
let submitBtnObjSnippet = `${options.name}SubmitBtn = new WmlButtonOneParams({
...
function addDescribeBlockToCpntSpecFile(
  ...
  if(!startNode){
    targets = findNodeTypeByRecursion(new FindNodeTypeByRecursionParams({
      node: sourceFile,
...
describe('init${optionString.capitalize(false)}', () => {
    ...
    does the required action\`, () => {
      ...
      // assert
      expect(cpnt.${options.name}).toBeInstanceOf(WMLForm);
    });
  });`
  let spacingPrefix = "\n  "
