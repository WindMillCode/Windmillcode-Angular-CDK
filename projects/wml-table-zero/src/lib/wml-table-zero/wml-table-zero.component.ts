// angular
import { ChangeDetectionStrategy, ChangeDetectorRef, Component, HostBinding, OnInit,  Input, Renderer2, ViewContainerRef, ComponentRef, ViewEncapsulation, ElementRef   } from '@angular/core';

// rxjs
import { Observable, Subject, fromEvent, of } from 'rxjs';
import { delay, distinctUntilChanged, map, startWith, takeUntil,tap } from 'rxjs/operators';
import { WMLAPIPaginationRequestModel, WMLAPIPaginationResponseModel, WMLCustomComponent,  WMLUIProperty,addCustomComponent,fillMissingProperties,WMLDeepPartial } from '@windmillcode/angular-wml-components-base';
...
import { WmlTableZeroSelectPageComponent, WmlTableZeroSelectPageParams } from '../wml-table-zero-select-page/wml-table-zero-select-page.component';
import { WmlTableZeroPopupOverlayComponent, WmlTableZeroPopupOverlayParams } from '../wml-table-zero-popup-overlay/wml-table-zero-popup-overlay.component';
import { WmlTableZeroPageSizeComponent, WmlTableZeroPageSizeParams } from '../wml-table-zero-page-size/wml-table-zero-page-size.component';
import { WmlTableZeroFilterComponent, WmlTableZeroFilterParams } from '../wml-table-zero-filter/wml-table-zero-filter.component';
import { WmlTableZeroSortComponent, WmlTableZeroSortParams } from '../wml-table-zero-sort/wml-table-zero-sort.component';
import {  WMLTableZeroTablePredicateReturnValue } from '../models';
import { WmlTableZeroDropdownItemComponent, WmlTableZeroDropdownItemParams } from '../wml-table-zero-dropdown-item/wml-table-zero-dropdown-item.component';
import { WmlTableZeroRowTypeComponent, WmlTableZeroRowTypeParams } from '../wml-table-zero-row-type/wml-table-zero-row-type.component';
...
export class WmlTableZeroComponent  {
  ...
  classPrefix = this.generateClassPrefix('WmlTableZero')
  @Input('params') params: WmlTableZeroParams = new WmlTableZeroParams()
  // @Input('params') params!: WmlTableZeroParams
  ...
  closePopup=(evt?)=>{
    if(evt){
      if(evt.target.className !== "WmlPopupMainPod"){
        return
      }
    }
  ...
  openPopup(cpntInfo:WMLCustomComponent) {
    let popupParams = new WMLPopupParams({
      cpnt : cpntInfo.cpnt,
      params : cpntInfo.params ?? {}
    })
    this.overlayRef = addCustomComponent(
      this.vcr,
      WmlTableZeroPopupOverlayComponent,
      new WmlTableZeroPopupOverlayParams({
  ...
  updateTable(obs$?:Observable<{req:WMLAPIPaginationRequestModel, res:WMLAPIPaginationResponseModel}>){
    return obs$?.pipe(
      ...
      map(({req,res})=>{
        this.params.instructionStackValues.unshift({req,res})
        if(this.params.instructionStackValues.length >= this.params.instructionStackLimit){
          this.params.instructionStackValues.pop()
        }
        return res
      }),
      tap((res)=>{

        this.params.displayRows = res.data
        this.cdref.detectChanges()
      })
    )
  }
  initTable(){
    ...
    return this.updateTable(this.params.performTableAction())
    ?.pipe(
      takeUntil(this.ngUnsub),
      tap(()=>{
        this.params.changeRowType(this.params.currentRowType)
  ...
  listenForSelectPage = ()=>{
    return this.listenForTableEvent(this.updatePageNumberSubj,this.params.selectPage)
  }
  listenForPageSize = ()=>{
    return this.listenForTableEvent(this.updatePageSizeSubj,this.params.selectPageSize)
  }
  listenForFilter = ()=>{
    return this.listenForTableEvent(this.updateFilterSubj,this.params.filterTable)
  }
  listenForSort = ()=>{
    return this.listenForTableEvent(this.updateSortSubj,this.params.sortTable)
  }
  listenForRowType =()=>{
    return this.listenForTableEvent(this.updateRowTypeSubj,this.params.changeRowType)
  }
  ...
  listenForTableControls =()=>{
    return this.params.tableControlFiredSubj
    .pipe(
      takeUntil(this.ngUnsub),
      tap((key)=>{
        let instructions =  this.params.instructionStackValues[0]
        switch (key) {
          case WmlTableZeroControlEnums.PREV_PAGE:
            this.updateTable(this.params.previousPage())?.subscribe()
            break;

          case WmlTableZeroControlEnums.NEXT_PAGE:
            this.updateTable(this.params.nextPage())?.subscribe()
            break;

          case WmlTableZeroControlEnums.SELECT_PAGE:
            this.openPopup(new WMLCustomComponent({
              cpnt:WmlTableZeroSelectPageComponent,
              params:new WmlTableZeroSelectPageParams({
                title:this.params.textContent.selectPage.title,
                pages:this.params.instructionStackValues[0].res.totalPages,
          ...
          case WmlTableZeroControlEnums.PAGE_SIZE:
            this.openPopup(new WMLCustomComponent({
              ...
              params:new WmlTableZeroPageSizeParams({
                title:this.params.textContent.pageSize.title,
                pageSize:this.params.instructionStackValues[0].req.pageSize,
          ...
          case WmlTableZeroControlEnums.FILTER:
            this.openPopup(new WMLCustomComponent({
              ...
              params:new WmlTableZeroFilterParams({
                ...this.params.textContent.filter,
                filter:this.params.instructionStackValues[0].req.filter,
            ...
            case WmlTableZeroControlEnums.SORT:
              this.openPopup(new WMLCustomComponent({
                ...
                params:new WmlTableZeroSortParams({
                  ...this.params.textContent.sort,
                  sort:this.params.instructionStackValues[0].req.sort,
            ...
            case WmlTableZeroControlEnums.ROW_TYPE:
              this.openPopup(new WMLCustomComponent({
                ...
                params:new WmlTableZeroRowTypeParams({
                  ...this.params.textContent.rowType,
  ...
  changeRowTypeBasedOnWindowSize =()=>{
    if(!this.params.changeRowTypeBasedOnWindowSizeIsPresent){
      return of()
    }
    ...
    .pipe(
      ...
      tap(()=>{
        if(mobile.matches){
          this.params.changeRowType(0)
        }
        if(tablet.matches || desktop.matches){
          this.params.changeRowType(1)
        }
      })
    )
  ...
  ngAfterViewInit(){
    // this.params.tableControlFiredSubj.next(WmlTableZeroControlEnums.ROW_TYPE)
  }
  ...
  ngOnDestroy(){
    ...
    // @ts-ignore
    this.params.ngUnsub.next();
    // @ts-ignore
    this.params.ngUnsub.complete()
  }
...
export class WmlTableZeroParams {
  constructor(params:Partial<WmlTableZeroParams &{paramsTextContent:WMLDeepPartial<WmlTableZeroParams["textContent"]>}>={}){
    Object.assign(
      this,
      {
        ...params
    ...
    if(!params.tablePredicate){
      this.businessRows =Array(70)
      .fill(null)
      .map((nullVal,index0)=>{
        return  new WmlTableZeroItemParams({
          overwriteBusinessDataValues:true
        })
      })
    }
    fillMissingProperties(params.paramsTextContent ??{},this.textContent)

    this.tableControls= new WmlInfiniteDropdownParams({
  ...
  displayRows : WmlTableZeroItemParams[] = []
  businessRows: WmlTableZeroItemParams[]=[]
  textContent = {
    controlItems:{
      previous: "Previous",
      next: "Next",
  ...
  private currentPageOption = new WmlInfiniteDropdownOption({

    custom:new WMLCustomComponent<WmlTableZeroDropdownItemComponent,WmlTableZeroDropdownItemParams>({
      cpnt:WmlTableZeroDropdownItemComponent,
      params:new WmlTableZeroDropdownItemParams({
        paramTexts:[this.textContent.controlItems.page,""]
      })
    })
  })
  private currentPageSizeOption = new WmlInfiniteDropdownOption({

    custom:new WMLCustomComponent<WmlTableZeroDropdownItemComponent,WmlTableZeroDropdownItemParams>({
      cpnt:WmlTableZeroDropdownItemComponent,
      params:new WmlTableZeroDropdownItemParams({
        paramTexts:[this.textContent.controlItems.pageSize,""]
      })
    })
  })
  generateSpacingOption =()=>new WmlInfiniteDropdownOption({
    style:{
      height:"calc(35.6198/16 * 1rem)"
    }
  })
  tableControls!:WmlInfiniteDropdownParams
  ...
  private generateTableControlItems(){
    ...
    this.currentPageOption.custom.params.texts[0].text = page
    this.currentPageSizeOption.custom.params.texts[0].text = pageSize
  ...
  filterTable = (filters:{key:WmlInfiniteDropdownOption,value:string}[])=>{
    let current = this.instructionStackValues[0]
    let reqBody = new WMLAPIPaginationRequestModel(current.req)
  ...
  sortTable = (sorts:{key:WmlInfiniteDropdownOption,direction:string}[])=>{
    let current = this.instructionStackValues[0]
    let reqBody = new WMLAPIPaginationRequestModel(current.req)
    sorts.forEach((sort)=>{
  ...
  private updateCurrentPageCounter(reqBody: WMLAPIPaginationRequestModel) {
    this.currentPageOption.custom.params.texts[1].text =  (reqBody.pageNum + 1);
    this.currentPageOption.custom.params.detectChangeSubj.next()
    this.currentPageOption.detectChangeSubj.next()
  }
  private updateCurrentPageSize(reqBody: WMLAPIPaginationRequestModel) {
    this.currentPageSizeOption.custom.params.texts[1].text =  (reqBody.pageSize);
    this.currentPageSizeOption.custom.params.detectChangeSubj.next()
    this.currentPageSizeOption.detectChangeSubj.next()
  }
  filterPredicate = (infos:WMLAPIPaginationRequestModel["filter"], rows:WmlTableZeroItemParams[])=>{

    let finalRows:WmlTableZeroItemParams[] = []
  ...
  private removeInvalidRows= (infos:WMLAPIPaginationRequestModel["sort"], rows:WmlTableZeroItemParams[])=>{
  ...
  private checkIfAllRowTypesMatch(targetRows: WmlTableZeroItemParams[], infos: WMLAPIPaginationRequestModel["sort"]) {
  ...
  private generateSortOrders(infos: WMLAPIPaginationRequestModel["sort"], rows: WmlTableZeroItemParams[]):Array<ArraySortPredicate> {
  ...
  sortPredicate = (infos:WMLAPIPaginationRequestModel["sort"], rows:WmlTableZeroItemParams[])=>{
  ...
  tablePredicate = (reqBody = new WMLAPIPaginationRequestModel({pageSize:16}))=>{
    ...
    .pipe(
      takeUntil(this.ngUnsub),
      map((reqBody)=>{
        let rows:WmlTableZeroItemParams[] = []
        ...
        let resBody =  new WMLAPIPaginationResponseModel<WmlTableZeroItemParams>({
          data:pageRows,
